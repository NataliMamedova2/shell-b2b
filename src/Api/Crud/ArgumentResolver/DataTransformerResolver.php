...
final class DataTransformerResolver implements ArgumentValueResolverInterface
{
    /**
    ...
    public function supports(Request $request, ArgumentMetadata $argument): bool
    {
        ...
        $class = $this->getClassFromRoute($request->get('_route'), 'transformer');

        $reflection = new \ReflectionClass($argument->getType());
        if ($reflection->implementsInterface(DataTransformer::class) && empty($class)) {
            return false;
        }

        if ($reflection->implementsInterface(DataTransformer::class)) {
            return true;
        }
